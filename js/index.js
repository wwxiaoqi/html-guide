var proxy_url = "https://proxy.funcc.cc/-----";
...
var comm_list = [
    {
        ...
        list: [
            ...
            },
            {
                tag: "购物",
                link: [
                    { name: "淘宝", url: "http://t.cn/A67guUFK" },
                    { name: "京东", url: "https://url.cn/56gsrVX" },
                    { name: "拼多多", url: "http://t.cn/A67gnb0q" },
...
$(document).ready(function () {
    ...
    var switchState = Cookies.get("proxy");
    if (switchState === "1") {
...
(function ($) {
    ...
    function setActiveTab($tab) {
        ...
        var slug = $tab.attr("class");
        $tab.addClass("active");
        var content = "";
        var proxyValue = getCookie("proxy");
        $.each(comm_list, function (index, item) {
            if (slug == item.slug) {
                var list = item.list;
                $.each(list, function (index, item) {
                    content += "<ul><li>" + item.tag + "</li>";
                    var links = item.link;
    ...
    function getCookie(name) {
        ...
        return Cookies.get(name) || defaultValues[name];
    }
    ...
    function setSearchEngine(tab) {
        $(tab).addClass("active").siblings(".active").removeClass("active");
        var proxyValue = getCookie("proxy");
        var proxyString = proxyValue === "1" ? proxy_url : "";
        $(".search-hidden").remove();
        if ($(tab).hasClass("baidu")) {
            $(".search-form").attr(
                "action",
                proxyString + "https://www.baidu.com/s"
            );
            $(".search-keyword").attr({
                name: "word",
                placeholder: "百度一下，你就知道",
            });
        } else if ($(tab).hasClass("google")) {
            $(".search-form").attr(
                "action",
                proxyString + "https://www.google.com/search"
            );
            $(".search-keyword").attr({
                name: "q",
                placeholder: "Google 搜索",
            });
        } else if ($(tab).hasClass("bing")) {
            $(".search-form").attr(
                "action",
                proxyString + "https://cn.bing.com/search"
            );
            $(".search-keyword").attr({
                name: "q",
                placeholder: "微软 Bing 搜索",
            });
        } else if ($(tab).hasClass("image")) {
            $(".search-form").attr(
                "action",
                proxyString + "https://cn.bing.com/images/search"
            );
            $(".search-keyword").attr({
                name: "q",
                placeholder: "海量图片搜索",
            ...
            imageIcon.src =
                proxyString +
                "https://images.google.com/favicon.ico?" +
                Date.now();
        ...
        } else if ($(tab).hasClass("torrent")) {
            $(".search-form").attr(
                "action",
                proxyString + "https://torrentz2.nz/search"
            );
            $(".search-keyword").attr({
                name: "f",
                placeholder: "磁力链，种子搜索",
            });
        } else if ($(tab).hasClass("scholar")) {
            $(".search-form").attr(
                "action",
                proxyString + "https://xueshu.baidu.com/s"
            );
            $(".search-keyword").attr({
                name: "wd",
                placeholder: "中英文文献检索",
            });
            var scholarIcon = new Image();
            scholarIcon.src =
                "https://scholar.google.com/favicon.ico?" + Date.now();
            scholarIcon.onload = function () {
                $(".search-form").attr(
                    "action",
    ...
    setActiveTab($(".work-link").find(".tab span:first"));
    setSearchEngine($(".search-tab").find("span." + getCookie("search")));
    $(".work-link .tab").on("click", "span", function () {
        setActiveTab($(this));
    });
    $(".search-tab").on("click", "span", function () {
        setSearchEngine($(this));
        setCookie("search", this.className.split(" ")[0]);
    });
})(jQuery);
